...
par<-list(
		  aa=1, ab=0.00059, gmax=5000,  	# Autogenic
          ca=381.8, cb=2.4, cc=4.9, cd=0.84, ce=00, cf=187.8, cg=-4.9,  # Competition
          ta1=c(259.7,	269.8,	290.4,	284.8,	295.0,	301.6), 
          tb1=c(500,	257,	31.0,	235.8,	459,	64.0),
          pa1=c(337.3,	242.6,	9.7,	64.3,	192.1,	20.76), 
          pb1=c(740.5,	606.7,	235.22,	1338.8,	309.13,	377.6), 
          pc1=0.016, 
...
var <- list(SIZE = "BA.tree.cm2",  # Autogenic
            ...
            PRECIP = qupr.run[,precip.col.ind], FLOW="flow.acc"
...
par_hi <-list(
			  aa=1, ab=1000, gmax=5000, # Autogenic
...
for(i in 1:length(temp.col))
	{	plot(x.temp[,i],y.temp[,i],ylim=c(0,1), xlab="Tavg (K)",ylab="Temp Effect",type="l",lwd=2, main=temp.col[i])
		abline(v=min(qupr.run[,temp.col[i]]), col="red")
		abline(v=max(qupr.run[,temp.col[i]]), col="red")
	}
dev.off()

# # summary(rowMeans(qupr.run[,temp.col]))
...
# plot(x.temp, y.temp, type="l", lwd=2,ylim=c(0,1), xlab="Tavg (K)", ylab="Temp Effect")
		...
		# abline(v=max(rowMeans(qupr.run[,temp.col])), col="red")
# dev.off()

##########
# Precipitation
plot(qupr.run$BAI ~ rowSums(qupr.run[,precip.col]), xlab="Total Precip Mar-Oct", ylab="BAI (mm)")
...
for(i in 1:length(temp.col))
	{	plot(x.precip[,i],y.precip[,i], ylim=c(0,1), xlab="Precip (mm)", ylab="BAI", type="l", lwd=2, main=precip.col[i])
		abline(v=min(qupr.run[,precip.col[i]]), col="red")
		abline(v=max(qupr.run[,precip.col[i]]), col="red")
	}
dev.off()
...
# summary(rowSums(qupr.run[,precip.col]))
# summary(y.precip)
# dev.off()
...
# par(mar=c(5,5,4,1))
	...
	# abline(v=min(qupr.run[,temp.col]), col="black", lty="dashed")
	# abline(v=max(qupr.run[,temp.col]), col="black", lty="dashed")			
